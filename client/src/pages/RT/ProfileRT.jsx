...
import { Camera, User, Pencil, LoaderCircle, Save, X } from 'lucide-react'
import { BigDataContainer, DataContainer, FormInput, FormTextarea } from '../../components'
      ...
      <Form method='POST' encType='multipart/form-data' className='w-full h-full rounded-xl p-4'>
        ...
        <section className='w-full pb-20 rounded-2xl shadow-lg mt-10'>
          <div className='w-full h-[30vh] bg-gradient-to-b from-newBlue to-blue-500 rounded-t-2xl flex justify-end items-end gap-x-4 p-4 group'>
            ...
            { isEdit ? (
                <button type='submit' className={`bg-newBlue w-[12%] text-white text-center justify-center py-2 px-4 rounded-lg text-sm flex items-center gap-x-3 group duration-100 ease-in-out cursor-default select-none transition-opacity opacity-0 group-hover:opacity-100`}>
                  { isSubmitting ? <LoaderCircle className='w-4 h-4 stroke-white animate-spin'/> : <Save className='w-4 h-4 stroke-white'/> }
                  { isSubmitting ? 'Menyimpan' : 'Simpan' } 
                </button>
            ) : null }

            <Link to={`${isEdit ? '/rt/profil'  : '/rt/profil?edit=true'}`} className={`${isEdit ? 'bg-newRed/80 hover:bg-newRed text-white ' : 'bg-white hover:bg-slate-200 text-slate-600 hover:text-slate-950 '} w-[12%] text-center justify-center py-2 px-4 rounded-lg text-sm flex items-center gap-x-3 group duration-100 ease-in-out cursor-default select-none transition-opacity opacity-0 group-hover:opacity-100`}>
                ...
                <section className='w-full grid grid-cols-3 gap-x-6 gap-y-4 mt-6'>
                  ...
                  { !isEdit ? (
                    ...
                    <>
                      <DataContainer labelData='nama lengkap' valueData={user.nama} />
                      <DataContainer labelData='email' valueData={user.email} />
                      <FormInput inputName='nomor_hp' placeholder='masukan nomor telepon'labelInput='kontak' isAutoFocus={true} defaultValue={user.nomor_hp} />
                    </>
