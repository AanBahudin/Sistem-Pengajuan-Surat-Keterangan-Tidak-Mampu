...
import { AtSign, KeyRound, X, EyeClosed, Eye, LoaderCircle, Landmark } from 'lucide-react'
    ...
    <Form method='POST' className='w-full h-[100%] p-10 flex items-center justify-center'>
      ...
      <section className='w-full h-[100%] rounded-xl bg-slate-200 flex justify-between items-center shadow-lg'>
        <div className='w-full pl-20 '>
          ...
          <h1 className='text-slate-600 font-bold text-2xl'>Selamat Datang Kembali di Pengajuan SKTM</h1>
          <p className='text-slate-500 text-sm'>Silahkan login terlebih dahulu untuk masuk ke dalam website</p>
          ...
          <div className='w-[70%] mt-8 flex flex-col gap-y-4'>
            ...
            <div className=''>
              ...
              <div className='flex bg-slate-300 w-full h-12 items-center justify-between px-4 rounded-md'>
                <div className='flex items-center gap-x-4 w-full'>
                  ...
                  <input type="email" name='email' id='email' required className='w-full bg-transparent h-full text-sm outline-none flex-1' placeholder='email anda ...' autoFocus autoComplete='off' onChange={(e) => setEmail(e.target.value)} value={isEmail} />
                </div>
            ...
            <button type='submit' disabled={isSubmitting} className='bg-newBlue/70 hover:bg-newBlue/90 disabled:bg-newBlue cursor-default py-2 rounded-lg my-2 text-white font-medium duration-200 ease-in-out flex justify-center items-center gap-x-2'>
              ...
              <span>{ isSubmitting ? 'tunggu sebentar' : 'masuk' }</span>
            </button>
            <div className='flex items-start justify-start gap-x-2'>
              <input type="checkbox" name="aggrement" id="aggrement" required />
              <label htmlFor="aggrement" className='text-[12px] text-slate-700/80'>Dengan menyetujui proses login ini, Anda menerima dan setuju untuk mematuhi <span className='text-newBlue'> Ketentuan Layanan, Kebijakan Privasi, </span> serta semua aturan dan regulasi yang berlaku</label>
            </div>

            <p className='text-slate-900 text-[14px] text-center mt-4 leading-relaxed'>Belum punya akun? <Link to='register' className='text-newBlue underline leading-relaxed'>Daftar dsini</Link></p>
          </div>
        </div>
