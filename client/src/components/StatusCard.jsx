...
const StatusCard = ({ _id, nama, createdAt, kelurahan, statusAccRt, statusAccKelurahan, alasanPengajuan}) => {
  return (
    <Link to={`pengajuan/${_id}`} className='w-[350px] h-[30vh] rounded-xl border-slate-300 hover:border-newBlue/40 border-[2px] px-4 py-2 flex items-start justify-between flex-col cursor-default hover:shadow-xl duration-200 ease-in-out'>
      ...
      <p className={`text-sm ${statusAccKelurahan === 'terima' && statusAccRt === 'terima' ? 'bg-newBlue/70' : (statusAccKelurahan === 'tolak' || statusAccRt === 'tolak' ? 'bg-newRed/70' : 'bg-slate-500')} py-2 px-4 rounded-md text-center text-white mt-1`}>{statusAccKelurahan === 'terima' && statusAccRt === 'terima' ? 'Telah Diterima' : (statusAccKelurahan === 'tolak' || statusAccRt === 'tolak' ? 'Ajuan Tertolak' : 'Ajuan Pending')}</p>
      </div>
    </Link>
  )
}
